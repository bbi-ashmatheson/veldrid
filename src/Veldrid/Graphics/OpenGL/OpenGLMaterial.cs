...
namespace Veldrid.Graphics.OpenGL
{
    public class OpenGLMaterial : Material, IDisposable
    {
        ...
        {
            vb.Apply();
            var inputSlot = _inputsBySlot[vertexBufferSlot];
            int slotBaseIndex = GetSlotBaseIndex(vertexBufferSlot);
            for (int i = 0; i < inputSlot.Elements.Length; i++)
            {
                var element = inputSlot.Elements[i]; 
                int actualIndex = slotBaseIndex + i;
                GL.VertexAttribPointer(actualIndex, element.ElementCount, element.Type, element.Normalized, inputSlot.VertexSizeInBytes, element.Offset);
                i += 1;
            }
        }
